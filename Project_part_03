...
library(FinTS) #for heteroscedasticity tests
...
# Calculate Mean Squared Error (MSE) for the predictions on test
mse <- mean((predictions - test_data$AQ_nox)^2)
cat("Mean Squared Error:", mse, "\n")

# Calculate Root Mean Squared Error (RMSE) for the predictions on test
rmse <- sqrt(mse)
...
cat("RMSE for ARMA Model on train Residuals:", arma_train_rmse, "\n")
...
# ACF and PACF of residuals after ARMA 
...
# Calculate RMSE of standardized GARCH residuals for Bergamo on test set
BG_garch_rmse <- sqrt(mean(garch_resid^2))
cat("RMSE of standardized GARCH residuals on test set (BG):", BG_garch_rmse, "\n")

# Calculate RMSE of standardized GARCH residuals for Bergamo on train set
...
# Calculate the true RMSE for test set
...
# For MN dataset
# Calculate combined predictions and RMSE for test set
MN_rf_predictions_test <- predict(MN_rf_model, newdata = MN_test_data)
MN_arma_fitted_test <- as.numeric(fitted(MN_arma_fitted_test))
MN_garch_fitted_test <- as.numeric(fitted(MN_fit_test_garch))

MN_combined_predictions_rf_arma_test <- MN_rf_predictions_test + MN_arma_fitted_test
MN_final_predictions_test <- MN_combined_predictions_rf_arma_test + MN_garch_fitted_test

# Calculate RMSEs for MN test set
MN_rf_rmse_test <- sqrt(mean((MN_test_data$AQ_nox - MN_rf_predictions_test)^2))
MN_arma_rmse_test <- sqrt(mean((MN_test_data$AQ_nox - MN_combined_predictions_rf_arma_test)^2))
MN_garch_rmse_test <- sqrt(mean((MN_test_data$AQ_nox - MN_final_predictions_test)^2))

cat("\nMantova Test Set RMSEs:\n")
cat("Random Forest RMSE:", MN_rf_rmse_test, "\n")
cat("RF + ARMA RMSE:", MN_arma_rmse_test, "\n")
cat("RF + ARMA + GARCH RMSE:", MN_garch_rmse_test, "\n")

# Calculate combined predictions and RMSE for train set
MN_rf_predictions_train <- predict(MN_rf_model, newdata = MN_train_data)
MN_arma_fitted_train <- as.numeric(fitted(MN_arma_model))
MN_garch_fitted_train <- as.numeric(fitted(MN_fit_train_garch))

MN_combined_predictions_rf_arma_train <- MN_rf_predictions_train + MN_arma_fitted_train
MN_final_predictions_train <- MN_combined_predictions_rf_arma_train + MN_garch_fitted_train

# Calculate RMSEs for MN train set
MN_rf_rmse_train <- sqrt(mean((MN_train_data$AQ_nox - MN_rf_predictions_train)^2))
MN_arma_rmse_train <- sqrt(mean((MN_train_data$AQ_nox - MN_combined_predictions_rf_arma_train)^2))
MN_garch_rmse_train <- sqrt(mean((MN_train_data$AQ_nox - MN_final_predictions_train)^2))
